...
namespace Stride.Physics.Bepu
{
    ...
    public class BepuHelpers
    {
        ...
        internal static BepuPhysicsSystem physicsSystem;
        ...
        public static void AssureBepuSystemCreated()
        {
            ...
            if (physicsSystem == null)
            {
                ...
                physicsSystem = ServiceRegistry.instance.GetService<IPhysicsSystem>() as BepuPhysicsSystem;
                if (physicsSystem == null)
                {
                    ...
                    lock (creationLocker)
                    {
                        ...
                        if (physicsSystem == null)
                        {
                            physicsSystem = new BepuPhysicsSystem(ServiceRegistry.instance);
                            ...
                            physicsSystem.CreateBepu(null, PhysicsEngineFlags.None);
                        }
                    }
                }
                else
        ...
        public static bool SanityCheckShape(IShape shape)
        {
            ...
            if (shape is BepuPhysics.Collidables.Mesh mesh)
        ...
        /// </summary>
        public static IShape OffsetSingleShape<TShape>(TShape shape, Vector3? offset = null, Quaternion? rotation = null, bool kinematic = false) where TShape : unmanaged, IConvexShape
        {
                ...
                using (BepuSimulation.instance.simulationLocker.WriteLock())
                {
                    if (kinematic)
                    {
                     //   compoundBuilder.AddForKinematicEasy(shape, new BepuPhysics.RigidPose(ToBepu(offset ?? Vector3.Zero), ToBepu(rotation ?? Quaternion.Identity)), 1f);
                        ...
                        compoundBuilder.AddForKinematic<TShape>(shape, new BepuPhysics.RigidPose(ToBepu(offset ?? Vector3.Zero), ToBepu(rotation ?? Quaternion.Identity)), 1f);
                    }
                    else
                    {
                        compoundBuilder.Add<TShape>(shape, new BepuPhysics.RigidPose(ToBepu(offset ?? Vector3.Zero), ToBepu(rotation ?? Quaternion.Identity)), 1f);
                    }
        ...
        //    if (allowOffsetCompound && center.LengthSquared() > 0.01f) return OffsetSingleShape<Box>(box, center);
            return box;
        }
        ...
        private static unsafe bool getMeshOutputs(Stride.Rendering.Mesh modelMesh, out List<Vector3> positions, out List<int> indicies)
        {
            ...
            object verts = smd.VertexBuffers;
            ...
            indicies = new List<int>(smd.IndexBuffer.Count);
            //      for (int i = 0; i < smd.IndexBuffer.Count; i++)
            //        indicies.Add((int)smd.IndexBuffer[i]);

            var indexOffset = (uint)positions.Count / 3;
            //var indexOffset = (uint)combinedVerts.Count / 3;

            var indexBufferRef = AttachedReferenceManager.GetAttachedReference(smd.IndexBuffer.Buffer);
            byte[] indexData;
            ...
            var indexIndex = smd.IndexBuffer.Offset;
            for (var v = 0; v < smd.IndexBuffer.Count; v++)
            {
        ...
        {
            public Stride.Rendering.Mesh mesh;
                                                                 ...
                                                                 float friction = 0.5f, float maximumRecoverableVelocity = 1f, SpringSettings? springSettings = null, bool disposeOnDetach = false)
        {
            ...
            else
            {
                ...
                Stride.Core.Threading.Dispatcher.For(0, scs.Length, (index) =>
                {
                    ...
                    {
                        sc.ColliderShape = new BepuPhysics.Collidables.Mesh(buf, scalev3, sc.PoolUsedForMesh);
                    }
                });
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe Stride.Core.Mathematics.Vector3 ToXenko(System.Numerics.Vector3 v)
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe Stride.Core.Mathematics.Quaternion ToXenko(System.Numerics.Quaternion q)
        {
